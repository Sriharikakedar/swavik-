...
const Navbar: React.FC<NavbarProps> = ({ isAuthenticated, setIsAuthenticated, user }) => {
  ...
  return (
    <header className={isAuthenticated
      ? "fixed top-0 right-0 w-[calc(100%-16rem)] z-40 flex items-center justify-end px-8 py-4 bg-theme-bg/80 backdrop-blur-md border-b border-theme-border"
      : "fixed top-0 left-0 right-0 z-40 flex items-center justify-between px-8 py-4 bg-theme-bg/80 backdrop-blur-md border-b border-theme-border"}>

      {!isAuthenticated && (
        <div>
          <Link to="/" className="text-4xl font-black text-theme-text hover:text-theme-accent transition-colors">
            ResolveX
          </Link>
        </div>
      )}

      <div className="flex items-center space-x-8 text-sm font-medium uppercase tracking-widest text-theme-dim">
        {!isAuthenticated && (
        ...
        {isAuthenticated ? (
          <>
            {(user || auth.currentUser) && (
              <div className="relative">
                <div
                  className="flex items-center gap-4 pl-4 border-l border-theme-border cursor-pointer"
                  onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                >
                  <div className="flex flex-col items-end">
                    <span className="text-[10px] text-theme-text font-bold tracking-tighter truncate max-w-[120px] normal-case">
          ...
          <>
            ...
            <button
              ...
              className="bg-theme-accent hover:bg-orange-dark text-white px-6 py-2 rounded-full transition-all"
            >
              Sign Up
            </button>
          </>
...
const Sidebar: React.FC = () => {
  ...
  return (
    <aside className="w-64 fixed top-0 left-0 h-screen bg-navy border-r border-navy/20 flex flex-col py-8 z-50">
      <div className="px-8 mb-12">
        <Link to="/" className="text-3xl font-black text-white hover:text-theme-accent transition-colors flex items-center gap-3">
      ...
      <nav className="flex flex-col space-y-1 px-4 flex-grow">
        <Link to="/dashboard" className={`flex items-center space-x-4 px-4 py-3 rounded-lg uppercase text-xs font-bold tracking-widest transition-all ${isActive('/dashboard')}`}>
          <i className="fas fa-columns w-4 text-center"></i>
          <span>Dashboard</span>
        </Link>
        <Link to="/search" className={`flex items-center space-x-4 px-4 py-3 rounded-lg uppercase text-xs font-bold tracking-widest transition-all ${isActive('/search')}`}>
          <i className="fas fa-search w-4 text-center"></i>
          <span>Search</span>
        </Link>
        <Link to="/analytics" className={`flex items-center space-x-4 px-4 py-3 rounded-lg uppercase text-xs font-bold tracking-widest transition-all ${isActive('/analytics')}`}>
        ...
        <Link to="/new-report" className={`flex items-center space-x-4 px-4 py-3 rounded-lg uppercase text-xs font-bold tracking-widest transition-all ${isActive('/new-report')}`}>
          <i className="fas fa-plus-square w-4 text-center"></i>
          <span>New Report</span>
        </Link>
      </nav>
      <div className="px-8 mt-auto flex flex-col space-y-6">
        <Link to="/settings" className={`flex items-center space-x-4 px-4 py-3 -mx-4 rounded-lg uppercase text-xs font-bold tracking-widest transition-all ${isActive('/settings')}`}>
...
const AppContent: React.FC = () => {
  ...
  return (
    <div className="flex min-h-screen bg-theme-bg text-theme-text transition-colors duration-300">
      ...
      <div className={`flex flex-col flex-grow min-h-screen ${isAuthenticated ? 'ml-64' : ''}`}>
