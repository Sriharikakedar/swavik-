...
import { generatePostmortem, sendEmail } from "../services/api";
...
const NewReport: React.FC = () => {
  ...
  const { aiDetection, autoParsing, incidentAlerts, alertEmail } = useSettings();
  ...
  const handleSeverityChange = (val: string) => {
    ...
    if (val.includes("P0")) setAssignee("SRE-Team");
    else if (val.includes("P1")) setAssignee("DevOps-Team");
    else setAssignee("Engineering-Team");
  };
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      const data = await generatePostmortem(logs, {
        alertEmail: incidentAlerts ? alertEmail : null,
        severity,
        title,
        assignee
      });
      ...
      // --- Trigger Email Alert for P0/P1 ---
      if (incidentAlerts && (severity.includes("P0") || severity.includes("P1"))) {
        try {
          await sendEmail({
            to: alertEmail,
            ...
            html: `
              <div style="font-family: sans-serif; padding: 20px; color: #1A2B4C; background-color: #F8F3ED; border-radius: 12px; border: 1px solid #D1603D10;">
                ...
                <br/>
                <a href="${window.location.origin}/report-view" style="display: inline-block; background-color: #1A2B4C; color: white; padding: 10px 20px; border-radius: 6px; text-decoration: none; font-weight: bold;">View Full Report</a>
          ...
          // Don't toast error for email failure to not distract from successful report generation
        }
      }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-2">
              ...
              >
                ...
                <optgroup label="SRE Team">
                  {users.map(u => (
                    <option key={u.id} value={u.name}>{u.name}</option>
                  ))}
                  <option value="SRE-Global-Oncall">SRE-Global-Oncall (Rotating)</option>
                </optgroup>
              </select>
            </div>
