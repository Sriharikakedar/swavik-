...
const ReportView: React.FC = () => {
  ...
  const { report, id, title, date, severity, assignee, priority, timeline, similarIncident, recommendedSOPs, resolutionTimeline } = location.state || {};
  ...
  if (!report) {
    return (
      <div className="text-center text-theme-text mt-20">
        No report available
      </div>
    );
  }
  ...
  const handleExport = () => {
    ...
    const bodySize = 10;
    const decorationColor = [26, 43, 76]; // #1A2B4C
    const accentColor = [209, 96, 61]; // #D1603D
    ...
    lines.forEach((line: string) => {
      ...
      if (isHeader) {
        ...
        doc.setFont("helvetica", "bold");
        doc.setTextColor(accentColor[0], accentColor[1], accentColor[2]);
        doc.text(listMarker, margin + listIndent, y);

        // Draw Text
        doc.setFont("helvetica", "normal");
        doc.setTextColor(60, 60, 60);
        const splitText = doc.splitTextToSize(text, contentWidth - textIndent);
        splitText.forEach((pLine: string, index: number) => {
          if (y > pageHeight - 25) { doc.addPage(); y = 35; }
          doc.text(pLine, margin + textIndent, y);
          y += 6;
        });
        y += 2; // Extra gap after list items
      } else {
        doc.setFont("helvetica", "normal");
        doc.setFontSize(bodySize);
        doc.setTextColor(60, 60, 60);
  ...
  return (
    <div className="max-w-4xl mx-auto px-8 py-12">
      ...
      {timeline && timeline.length > 0 && (
        <div className="mb-20">
          <div className="mb-8">
            ...
            <p className="text-theme-dim text-[10px] font-black uppercase tracking-widest mt-1">Automatic reconstruction of the event sequence</p>
          </div>
          ...
          <VerticalTimeline layout="1-column-left" lineColor="#1A2B4C20">
            {timeline.map((item: any, index: number) => (
              <VerticalTimelineElement
                ...
                iconStyle={{ background: '#D1603D', color: '#fff', boxShadow: '0 0 0 4px #F8F3ED, inset 0 2px 0 rgba(0,0,0,.08), 0 3px 0 4px rgba(0,0,0,.05)' }}
      ...
      <div className="flex gap-6 mb-20">
        <button
          onClick={handleEdit}
          className="px-8 py-3 bg-navy hover:bg-navy-light text-white rounded-xl font-bold transition-all shadow-lg shadow-navy/20"
        >
          Edit
        </button>
        <button
          onClick={handleExport}
          className="px-8 py-3 bg-theme-accent hover:bg-orange-dark text-white rounded-xl font-bold transition-all shadow-lg shadow-orange/20"
        >
          Export PDF
        </button>
        <button
          onClick={handleDelete}
          className="px-8 py-3 bg-theme-card hover:bg-red-50 text-red-500 border border-red-500/20 rounded-xl font-bold transition-all shadow-sm"
        >
          Delete
        </button>
      </div>
